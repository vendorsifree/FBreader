...
public class LibraryActivity extends TreeActivity implements
	...
	public void onCreate(Bundle icicle) {
		...
		final String selectedBookPath = getIntent().getStringExtra(
	...
	protected FBTree getTreeByKey(FBTree.Key key) {
		return key != null ? myLibrary.getLibraryTree(key) : myLibrary
				.getRootTree();
	}
	...
	protected void onListItemClick(ListView listView, View view, int position,
			long rowId) {
		final LibraryTree tree = (LibraryTree) getListAdapter().getItem(
				position);
		final Book book = tree.getBook();
		if (book != null) {
			showBookInfo(book);
		} else {
			if (tree instanceof Library.BookStoreTree) {
				SharedPreferences.Editor spe = getSharedPreferences(FBReader.BOOK_STORE_STORAGE, MODE_PRIVATE).edit();
				spe.putBoolean(FBReader.OPEN_BOOK_STORE, true);
				spe.commit();
				finish();
				HackedLog
						.i("----$$$----", "------------------> Open BookStore");
	...
	protected void onActivityResult(int requestCode, int returnCode,
		...
		if (requestCode == BOOK_INFO_REQUEST && intent != null) {
			final String path = intent
					.getStringExtra(BookInfoActivity.CURRENT_BOOK_PATH_KEY);
	...
	// Search
	//
	static final ZLStringOption BookSearchPatternOption = new ZLStringOption(
			"BookSearch", "Pattern", "");

	private void openSearchResults() {
		final FBTree tree = myLibrary.getRootTree().getSubTree(
				Library.ROOT_FOUND);
		if (tree != null) {
			openTree(tree);
		}
	...
	public void onCreateContextMenu(ContextMenu menu, View view,
		...
		final Book book = ((LibraryTree) getListAdapter().getItem(position))
				.getBook();
		if (book != null) {
	...
	private void createBookContextMenu(ContextMenu menu, Book book) {
		...
		menu.add(0, OPEN_BOOK_ITEM_ID, 0, resource.getResource("openBook")
				.getValue());
		menu.add(0, SHOW_BOOK_INFO_ITEM_ID, 0,
				resource.getResource("showBookInfo").getValue());
		if (book.File.getPhysicalFile() != null) {
			menu.add(0, SHARE_BOOK_ITEM_ID, 0, resource
					.getResource("shareBook").getValue());
		}
		if (myLibrary.isBookInFavorites(book)) {
			menu.add(0, REMOVE_FROM_FAVORITES_ITEM_ID, 0,
		...
		if (myLibrary.canRemoveBookFile(book)) {
			menu.add(0, DELETE_BOOK_ITEM_ID, 0,
					resource.getResource("deleteBook").getValue());
		}
	}
	...
	private MenuItem addMenuItem(Menu menu, int index, String resourceKey,
			int iconId) {
		final String label = LibraryUtil.resource().getResource("menu")
	...
	private void deleteBook(Book book, int mode) {
		...
		if (getCurrentTree() instanceof FileTree) {
			getListAdapter().remove(
					new FileTree((FileTree) getCurrentTree(), book.File));
		} else {
		...
		runOnUiThread(new Runnable() {
			public void run() {
				...
				default:
					...
					break;
				case StatusChanged:
					setProgressBarIndeterminateVisibility(!myLibrary
							.isUpToDate());
					break;
				case Found:
					openSearchResults();
					break;
				case NotFound:
					UIUtil.showErrorMessage(LibraryActivity.this,
							"bookNotFound");
					break;
				}
			}
